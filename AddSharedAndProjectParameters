...
# Create Shared Parameters from Excel and bind them to categories as Project Parameters
...
for param in params:  # All data are strings - will be converted accordigly
    parName = param[0]  # Gets String for Parameter Name
    parGroupName = param[1]  # Gets String for add SharedParameter into a group
    parType = param[2]  # String used for getting ParameterType from list
    parVisible = param[3]  # Will be converted to bool
    parCaterories = param[4]  # Will be converted to CategorySet
    parGroup = param[5]  # Gets String
    instance = param[6]  # Will be converted to bool
    parDescription = param[7]  # Gets String
    ...
    try:
        ...
        if not groups.Item[parGroupName]:
            groups.Create(parGroupName)  # Creates group if dose not exist
